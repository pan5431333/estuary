...
//  val (feature, label) = new GasCensorDataReader().read("/Users/mengpan/Downloads/NewDataset/training.*")
  ...
  val trainedModel = nnModel.multiNodesParTrain(AdamAkkaParallelOptimizer(learningRate = 0.0001))
//  val trainedModel = nnModel.train(feature, label, AdamOptimizer())

  val (testFeature, testLabel) = new GasCensorDataReader().read("/Users/mengpan/Downloads/CatData/test.*")
  val yPredicted = trainedModel.predictToVector(testFeature, Vector(1, 2, 3, 4, 5))
  val testAccuracy = Model.accuracy(Model.convertMatrixToVector(testLabel.map(_.toInt), Vector(1, 2, 3, 4, 5)), yPredicted)
