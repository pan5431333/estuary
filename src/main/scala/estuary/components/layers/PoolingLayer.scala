...
class PoolingLayer(val poolSize: Int, val stride: Int, val pad: Int, val poolType: PoolType)
  ...
  protected[estuary] lazy val filter: Filter = Filter(poolSize, pad, stride, preConvSize.channel, preConvSize.channel)
  protected[estuary] lazy val outputConvSize: ConvSize = calConvSize(preConvSize, filter)
  protected[estuary] lazy val maskMatrix: Array[DenseMatrix[Double]] = new Array[DenseMatrix[Double]](preConvSize.channel)
...
object PoolingLayer {
  ...
  implicit val poolingLayerCanSetParamNone: CanSetParam[PoolingLayer, None.type] = (_, _) => None

  implicit val poolingLayerCanSetParamMatrix: CanSetParam[PoolingLayer, DenseMatrix[Double]] = (_, _) => None

  implicit val poolingLayerCanExportParamNone: CanExportParam[PoolingLayer, None.type] = (_) => None

  implicit val poolingLayerCanExportParamMatrix: CanExportParam[PoolingLayer, Option[DenseMatrix[Double]]] = (_) => None
