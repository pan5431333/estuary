...
/**
  ...
  */
trait ClassicLayer extends Layer with LayerLike[ClassicLayer] with Activator with Trainable {
...
object ClassicLayer {
  ...
  implicit val classicLayerCanExportParam: CanExportParam[ClassicLayer, Option[DenseMatrix[Double]]] =
    (from) => {
      val w = from.param._1
      val b = from.param._2
      Some(DenseMatrix.vertcat(w, b.toDenseMatrix))
    }
  ...
  implicit val classicLayerCanForwardForPrediction: CanForward[ClassicLayer, ForPrediction[DenseMatrix[Double]], DenseMatrix[Double]] =
    (input, by) => {
      by.forward(input.input)(classicLayerCanForward)
    }

  implicit val classicLayerCanBackward: CanBackward[ClassicLayer, DenseMatrix[Double], (DenseMatrix[Double], Option[DenseMatrix[Double]])] =
    (input, by, regularizer) => {
      ...
      (dYPrevious, Some(grads))
    }

  implicit val classicLayerCanRegularize: CanRegularize[ClassicLayer] =
    (foor: ClassicLayer, regularizer: Option[Regularizer]) => {
      regularizer match {
        case None => 0.0
