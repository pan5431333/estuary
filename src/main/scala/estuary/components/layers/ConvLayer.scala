...
trait ConvLayer extends Layer with LayerLike[ConvLayer] with Activator with Trainable {
  ...
  protected[estuary] lazy val outputConvSize: ConvSize = ConvLayer.calConvSize(preConvSize, param)
...
object ConvLayer {
  ...
  implicit val convLayerCanExportParam: CanExportParam[ConvLayer, Option[DenseMatrix[Double]]] =
    (from) => {
      Some(DenseMatrix.vertcat(from.param.w, from.param.b.toDenseMatrix))
    }
  ...
  implicit val convLayerCanBackward: CanBackward[ConvLayer, DenseMatrix[Double], (DenseMatrix[Double], Option[DenseMatrix[Double]])] =
    (input, by, regularizer) => {
      ...
      (dYPreviousIm, Some(grads))
    }

  implicit val convLayerCanRegularize: CanRegularize[ConvLayer] =
    (foor: ConvLayer, regularizer: Option[Regularizer]) => {
      regularizer match {
        case None => 0.0
