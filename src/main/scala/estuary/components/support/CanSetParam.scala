...
//    }
  ...
  implicit def canSetParamLayer = new CanSetParam[Layer, DenseMatrix[Double]] {

    override def set(from: DenseMatrix[Double], foor: Layer): Unit = foor match {
      case c: ClassicLayer => c.setParam(from)
      case con: ConvLayer => con.setParam(from)
      case d: DropoutLayer => d.setParam(from)
      case p: PoolingLayer => p.setParam(from)
      case _ => throw new UnsupportedOperationException(s"Backward on $foor is unsupported")
    }
