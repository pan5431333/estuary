...
import org.mengpan.deeplearning.model.{LogisticRegressionModel, Model, ShallowNeuralNetworkModel}
...
/**
  ...
  */
object ClassTwoShallowNeuralNetworkDemo extends App{
  // Dataset Download Website: http://archive.ics.uci.edu/ml/machine-learning-databases/00224/
  //加载Gas Censor的数据集
  val catData: DlCollection[GasCensor] = GasCensorDataHelper.getAllData

  //归一化数据特征矩阵
  val normalizedCatData = NormalizeUtils.normalizeBy(catData){col =>
  ...
  //初始化LR的算法模型
  val nnModel: Model = new ShallowNeuralNetworkModel()
    .setNumHiddenUnits(256)
    .setHiddenActivationFunc(MyDict.ACTIVATION_RELU)
